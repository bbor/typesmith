...
/*
  ...
  Each item must have a `name`, or you can provide it in the parameters.
...
*/
typesmith.prototype.add_to_db = function(records, name, uid){
  records = records || {};
  ...
  for (var i_record = 0; i_record < records.length; i_record++)
  {
    ...
    assert(record.name, "This record needs a name!" + JSON.stringify(record));
